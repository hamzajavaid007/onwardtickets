// In-memory blog storage for fallback when MongoDB is unavailable
// This allows the blog CMS to work even without a database connection
...
// Global in-memory storage (persists during server runtime)
let inMemoryBlogs: BlogPost[] = [];

export function getBlogs(): BlogPost[] {
  return inMemoryBlogs.length > 0 ? inMemoryBlogs : DEFAULT_BLOGS;
}
...
export function updateBlog(id: string, updates: Partial<BlogPost>): boolean {
  const index = inMemoryBlogs.findIndex((b) => b._id === id);
  if (index === -1) return false;

  inMemoryBlogs[index] = { ...inMemoryBlogs[index], ...updates, updatedAt: new Date() };
  return true;
}
...
export function findBlogById(id: string): BlogPost | undefined {
  return inMemoryBlogs.find((b) => b._id === id);
}

export function isUsingDefaults(): boolean {
  return inMemoryBlogs.length === 0;
}
